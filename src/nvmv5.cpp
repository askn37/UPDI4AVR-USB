 ...
 * NOTE:
 *
 * UPDI NVM version 5 is used in the AVR-EB series.
 ...
 * 
 * - The signature is at address 0x1080.
 * 
...
namespace NVM::V5 {
  ...
  bool write_words_flash (uint32_t _dwAddr, size_t _wLength) {
    D2PRINTF(" NVM_V5_FLPERW=%06lX\r\n", _dwAddr);
    return (
      nvm_ctrl_change(0x00)
      && UPDI::send_words_block(_dwAddr, _wLength)
      && nvm_ctrl_change(0x05)  /* NVM_V5_CMD_FLPERW */
      && (nvm_wait() & 0x73) == 0
    );
  }
  ...
  size_t prog_init (void) {
    nvm_ctrl_change(0x0F);      /* NVM_V5_CMD_FLPBCLR */
    nvm_ctrl_change(0x1F);      /* NVM_V5_CMD_EEPBCLR */
